...
"""Cloud SQL PostgreSQL backend (via the Cloud SQL Python Connector)."""

# Import the fork's Postgres overrides so DVT-specific methods
# (raw_column_metadata, _metadata, ...) are active on this subclass too.
...
class CloudSQLPostgresBackend(PostgresBackend):
    name = "cloudsql_postgres"
    ...
    def do_connect(
        ...
        **kwargs,
        ...
        # Dummy URL: the dialect selects the pg8000 driver; the creator is
        # authoritative for opening each connection through the Connector.
        self.con = sa.create_engine(
            "postgresql+pg8000://", creator=creator, pool_pre_ping=True
        )

        # Equivalent of super().do_connect() below (mirrors
        # third_party.ibis.ibis_postgres.client.do_connect).
